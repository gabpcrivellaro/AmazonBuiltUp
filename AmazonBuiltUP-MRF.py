...
for i in sede_id:
    ...
    #Cria um dicionario com a matriz de erro da validação e exporta no GDrive
    MAT_fe = ee.Feature(None,{'Matriz de erro':CM_T})
    N_MAT = s + '_Matriz_Teste' 
    task_MAT = ee.batch.Export.table.toDrive(collection = ee.FeatureCollection(MAT_fe), description = N_MAT, folder = nome_folder_Gdrive) 
    task_MAT.start() 
    ...
    #Cria um dicionario com a acuracia global, do consumidor  e produtor e tranforma em uma tabela
    AT_fe = ee.Feature(None,{'Acurácia Global':acc_glob_t,'Acurácia do produtor':(prod_accuracy_t),'Acurácia do Consumidor':(consu_accuracy_t)})
    ...
    #Cria um dicionario com a matriz de erro da validação e exporta no GDrive
    MAV_fe = ee.Feature(None,{'Matriz de erro':CM_V})
    N_MAV = s + '_Matriz_validação' 
    task_MAV = ee.batch.Export.table.toDrive(collection = ee.FeatureCollection(MAV_fe), description = N_MAV, folder = nome_folder_Gdrive) 
    task_MAV.start()
    ...
    kappa_V = CM_V.kappa()
    #Cria um dicionario com a acuracia global, consumidor, produtor e o kappa e transforma em uma tabela
    AV_fe = ee.Feature(None,{'Acurácia Global':acc_glob_V,'Acurácia do produtor':(prod_accuracy_V),'Acurácia do Consumidor':(consu_accuracy_V),'Coeficiente de Kappa':(kappa_V)})
    #Exporta a tabela com a acuracia global e o kappa para o Gdrive
    ...
    #Mostra uma mensagem informando o fim do processamento da sede municipal "n"
    print("Processamento de ",Sede_nome,"Concluído!" )
    print()
